...
namespace ECommerceLiteUI.LogManaging
{
    public static class Logger
    {
        ...
        public static void LogMessage(string message,string page="-",string user="-")
        {
            try
            {
                string fileName = "ECommerceLite303_Logs_" + DateTime.Now.ToString("yyyMMdd") + ".txt";
                ...
                writer.Flush();
                writer.WriteLine($"{DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss")}" + $"\t User: {user} \t Page: {page} \t Message: {message}");
                writer.Close();
            }
            catch (Exception)
            {
