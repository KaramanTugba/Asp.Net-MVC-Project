    ...
    public class ProductController : Controller
    {
        ...
        public ActionResult Create(ProductViewModel model)
        {
            try
            {
                ...
                //Üürn tabloya kayıt olacak
                //todo:map leme yapılacak
                ...
                if (insertResult>0)
                {
                    ...
                    //acaba bu producta resim seçmiş mi?resim seçtiyse o resimlerin yllarını kaydet
                    if (model.Files.Any())
                    {
                        ...
                        foreach (var item in model.Files)
                        {
                            ...
                            if (item!=null&& item.ContentType.Contains("image")&&item.ContentLength>0)
                            {
                                ...
                                if (counter == 4)
                                {
                                    productPicture.ProductPicture4 = $"ProductPictures/{filename}/{model.ProductCode}/" + filename + "-" + counter + "-" + guid + extensionName;
                                }
                                if (counter == 5)
                                {
                                    productPicture.ProductPicture5 = $"ProductPictures/{filename}/{model.ProductCode}/" + filename + "-" + counter + "-" + guid + extensionName;
                                }
                            }
                            counter++;
                        }
                        //todo: yukarıyı for a dönüştürebilir miyiz?
                        ...
                        {
                            ModelState.AddModelError("", "Ürün eklendi ama ürüne ait fotoğraflar eklenirken beklenmedik bir hata oluştu.Ürününüzün fotoğraflarını daha sonra tekrar eklemeyi deneyebilirsiniz...");
                            return View(model); 
                        }
                    }
                    else
